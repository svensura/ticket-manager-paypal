...
app.post('/pay', async (req,res) => {
  ...
  // console.log('gigFeePPEur: ',gigFeePPEur )
  ...
  const execute_payment_json = {
    ...
    paypal.payment.execute(paymentId, execute_payment_json, async (error, payment) => {
        ...
        // payment went through   
            ...
            if (response.ok) { // if HTTP-status is 200-299
              // get the response body (the method explained below)
              const plural = (ticketAmount > 1) ? "s" : ""
              res.render('pages/message', { message: `${ticketAmount} Ticket${plural} wurden erfolgreich erworben. Ihre email-Adresse bzw. Ihr Phantasiename werden an der Abendkasse hinterlegt`})
            } else {
...
app.get('/cancel/:houseNo', async (req, res) => {
  //console.log("CANCEL");
  const body = { amount: ticketAmount * (-1), buyer: buyer  };
            const response = await fetch(`${API_URL}/gigs_buy/${gigId}`, {
              method: 'patch',
